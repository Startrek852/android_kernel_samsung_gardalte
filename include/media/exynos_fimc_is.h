...
#elif defined(CONFIG_VIDEO_EXYNOS4_FIMC_IS2)
#define FIMC_IS_MODULE_NAME			"exynos4-fimc-is2"
...
enum exynos4_flite_id {
	FLITE_ID_A = 0,
	FLITE_ID_B
};

enum exynos4_sensor_position {
	SENSOR_POSITION_REAR = 0,
	SENSOR_POSITION_FRONT
};
enum exynos4_sensor_id {
...
struct exynos4_sensor_power_info {
...
enum exynos4_sensor_channel {
	SENSOR_CONTROL_I2C_CH0	 = 0,
	SENSOR_CONTROL_I2C_CH1	 = 1
};
...
struct exynos4_sensor_gpio_info {
 ...
 * image sensor information required for host interace configuration.
 */

struct exynos4_fimc_is_sensor_info {
	char sensor_name[FIMC_IS_MAX_NAME_LEN];
	enum exynos4_sensor_position sensor_position;
	enum exynos4_sensor_id sensor_id;
	enum exynos4_csi_id csi_id;
	enum exynos4_flite_id flite_id;
	enum exynos4_sensor_channel i2c_channel;
	struct exynos4_sensor_power_info sensor_power;
	struct exynos4_sensor_gpio_info sensor_gpio;
	int max_width;
	int max_height;
	int max_frame_rate;
...
/* platform specific clock functions */
extern int exynos4_fimc_is_cfg_clk(struct platform_device *pdev);
extern int exynos4_fimc_is_clk_on(struct platform_device *pdev, int sensor_id);
extern int exynos4_fimc_is_clk_off(struct platform_device *pdev, int sensor_id);
extern int exynos4_fimc_is_sensor_power_on(struct platform_device *pdev,
							int sensor_id);
extern int exynos4_fimc_is_sensor_power_off(struct platform_device *pdev,
							int sensor_id);
#endif

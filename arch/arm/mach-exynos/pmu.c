...
void __iomem *exynos3470_list_automatic_wake[] = {
	EXYNOS_ARM_CORE_OPTION(0),
	EXYNOS_ARM_CORE_OPTION(1),
	EXYNOS_ARM_CORE_OPTION(2),
	EXYNOS_ARM_CORE_OPTION(3),
...
static void exynos3470_init_pmu(void)
{
	...
	for (i = 0 ; i < ARRAY_SIZE(exynos3470_list_automatic_wake) ; i++) {
		tmp = __raw_readl(exynos3470_list_automatic_wake[i]);
		tmp &= EXYNOS3470_AUDOMATIC_WAKEUP_MASK;
		__raw_writel(tmp, exynos3470_list_automatic_wake[i]);
	}
}
...
static int __init exynos_pmu_init(void)
{
	...
	if (soc_is_exynos4210()) {
		...
		pr_info("EXYNOS4270 PMU Initialize\n");

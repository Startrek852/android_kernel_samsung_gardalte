...
#define DWMCI_DDR200_RDDQS_EN(x)		(0x110 + (x))
#define DWMCI_DDR200_ASYNC_FIFO_CTRL(x)		(0x114 + (x))
#define DWMCI_DDR200_DLINE_CTRL(x)		(0x118 + (x))
/* DDR200 RDDQS Enable*/
...
#define DWMCI_DDR200_RDDQS_EN_DEF(x, y) \
	(DWMCI_TXDT_CRC_TIMER_FASTLIMIT(x) | DWMCI_TXDT_CRC_TIMER_INITVAL(y))
#define DWMCI_DDR200_DLINE_CTRL_DEF(x, y) \
	(DWMCI_FIFO_CLK_DELAY_CTRL(x) | DWMCI_RD_DQS_DELAY_CTRL(y))
...
static void exynos_dwmci_set_io_timing(void *data, unsigned int tuning, unsigned char timing)
{
	...
	u32 sclkin, cclkin;
	u32 base_address;
	u32 fifo_clk_delay, rd_dqs_delay;
	u32 crc_t_fastlimit, crc_t_initval;
	...
	base_address = 0x70;
	rddqs = __raw_readl(host->regs + DWMCI_DDR200_RDDQS_EN(base_address));
	rddqs &= ~(DWMCI_RDDQS_EN);
	dline = __raw_readl(host->regs + DWMCI_DDR200_DLINE_CTRL(base_address));
	...
	if (timing == MMC_TIMING_MMC_HS200_DDR) {
		...
		if (!tuning) {
			rddqs = (DWMCI_DDR200_RDDQS_EN_DEF(crc_t_fastlimit, crc_t_initval)
				| DWMCI_RDDQS_EN);
			dline = DWMCI_DDR200_DLINE_CTRL_DEF(fifo_clk_delay, rd_dqs_delay);
		}
	...
	__raw_writel(rddqs, host->regs + DWMCI_DDR200_RDDQS_EN(base_address));
	__raw_writel(dline, host->regs + DWMCI_DDR200_DLINE_CTRL(base_address));
}
...
/*
 * dwmci platdata define for exynos4
 */
	...
	if ((soc_is_exynos4210()) || soc_is_exynos4212() ||
		...
		if (slot_id < ARRAY_SIZE(exynos5_dwmci_devs))
			...
			pr_err("%s: slot %d is not supported\n", __func__,
			       slot_id);
	} else if (soc_is_exynos4270() || soc_is_exynos3470()) {

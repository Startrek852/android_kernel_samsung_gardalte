...
static void exynos3470_set_asv_info_arm(struct asv_info *asv_inform, bool show_value)
{
	...
	for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
		...
		asv_inform->asv_volt[i].asv_value =
			exynos3470_apply_volt_offset(arm_asv_volt_info[i][target_asv_grp_nr + 1], ID_ARM);
...
static void exynos3470_set_asv_info_int(struct asv_info *asv_inform, bool show_value)
{
	...
	for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
		...
		asv_inform->asv_volt[i].asv_value =
			exynos3470_apply_volt_offset(int_asv_volt_info[i][target_asv_grp_nr + 1], ID_INT);
		asv_inform->asv_abb[i].asv_freq = int_asv_abb_info[i][0];

		if (int_asv_abb_info[i][target_asv_grp_nr + 1] == ABB_BYPASS)
			tmp = ABB_X100;
		else
			tmp = int_asv_abb_info[i][target_asv_grp_nr + 1];
	...
	for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
		...
		if (mif_lock == 2) {
			if (i < 2) asv_inform->asv_volt[i].asv_value = mif_asv_volt_info[i][target_asv_grp_nr + 1];
				...
				asv_inform->asv_volt[i].asv_value =
						exynos3470_apply_volt_offset(mif_asv_volt_info[i][target_asv_grp_nr + 1], ID_MIF);
			}
		} else {
			asv_inform->asv_volt[i].asv_value = exynos3470_apply_volt_offset(mif_asv_volt_info[i][target_asv_grp_nr + 1], ID_MIF);
		}
		asv_inform->asv_abb[i].asv_freq = mif_asv_abb_info[i][0];

		if (mif_asv_abb_info[i][target_asv_grp_nr + 1] == ABB_BYPASS)
			tmp = ABB_X100;
		else
			tmp = mif_asv_abb_info[i][target_asv_grp_nr + 1];
...
static void exynos3470_set_asv_info_g3d(struct asv_info *asv_inform, bool show_value)
{
	...
	for (i = 0; i < asv_inform->dvfs_level_nr; i++) {
		...
		asv_inform->asv_volt[i].asv_value = exynos3470_apply_volt_offset(g3d_asv_volt_info[i][target_asv_grp_nr + 1], ID_G3D);
		asv_inform->asv_abb[i].asv_freq = g3d_asv_abb_info[i][0];

		if (g3d_asv_abb_info[i][target_asv_grp_nr + 1] == ABB_BYPASS)
			tmp = ABB_X100;
		else
			tmp = g3d_asv_abb_info[i][target_asv_grp_nr + 1];

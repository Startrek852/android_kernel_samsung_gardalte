...
#define SYSMMU_PROPERTY_MAPPING(ipname, prop) {		\
	.pdev = &SYSMMU_PLATDEV(ipname),		\
	.prop = prop,					\
}
...
static struct sysmmu_resource_map sysmmu_resmap4270[] __initdata = {
	SYSMMU_RESOURCE_MAPPING(4,	2d,	2d_acp),
	SYSMMU_RESOURCE_MAPPING(4,	camif0, flite0),
	SYSMMU_RESOURCE_MAPPING(4,	camif1, flite1),
...
#endif
	/*
	 * If FIMC-LITEs are needed to be controlled by exynos4_device_fimc_is
	 * in FIMC-IS driver and by exynos_device_flite0/1/2 in FIMC-LITE
	 * driver, the following restrictions must be kept:
	 * - iommu_attach_device() call against exynos4_device_fimc_is,
	 *   exynos_device_flite0, exynos_device_flite1 and exynos_device_flite2
	 *   is mutually exclusive.
	 * - platform_set_sysmmu() call against exynos4_device_fimc_is must be
...
static int __init init_sysmmu_platform_device(void)
{
	...
	if (soc_is_exynos4270() || soc_is_exynos3470()) {
		resmap[1] = sysmmu_resmap4270;
		nmap[1] = ARRAY_SIZE(sysmmu_resmap4270);
	}
...
static int __init setup_sysmmu_owner(void)
{
	if (soc_is_exynos5250())
		exynos5_sysmmu_init();
		 ...
		 soc_is_exynos4210() || soc_is_exynos4270() ||
		 soc_is_exynos3470())
		exynos4_sysmmu_init();

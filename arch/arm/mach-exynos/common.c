...
static const char name_exynos4270[] = "EXYNOS4270";
static const char name_exynos3470[] = "EXYNOS3470";
...
static struct cpu_table cpu_ids[] __initdata = {
	{
		...
		.idcode		= EXYNOS4270_CPU_ID,
		.idmask		= EXYNOS4_CPU_MASK,
		.map_io		= exynos4_map_io,
		.init_clocks	= exynos4_init_clocks,
		.init_uarts	= exynos_init_uarts,
		.init		= exynos_init,
		.name		= name_exynos4270,
	}, {
		.idcode		= EXYNOS3470_CPU_ID,
		.idmask		= EXYNOS4_CPU_MASK,
		.map_io		= exynos4_map_io,
		.init_clocks	= exynos4_init_clocks,
		.init_uarts	= exynos_init_uarts,
		.init		= exynos_init,
		.name		= name_exynos3470,
	}, {
...
static struct map_desc exynos4_iodesc[] __initdata = {
	{
		...
		.pfn		= __phys_to_pfn(EXYNOS4270_PA_PPMU_CPU),
		.length		= SZ_8K,
		.type		= MT_DEVICE,
	}, {
		...
		.pfn		= __phys_to_pfn(EXYNOS4270_PA_PPMU_DMC1),
		.length		= SZ_8K,
		.type		= MT_DEVICE,
	}, {
		.virtual	= (unsigned long)S5P_VA_BTS_CP,
		.pfn		= __phys_to_pfn(EXYNOS4_PA_BTS_CP),
		.length		= SZ_4K,
		.type		= MT_DEVICE,
	},
};
...
static struct map_desc exynos4270_iodesc[] __initdata = {
	{
		.virtual	= (unsigned long)S5P_VA_CMU_DMC,
		.pfn		= __phys_to_pfn(EXYNOS4270_PA_CMU_DMC),
		.length		= SZ_16K,
		.type		= MT_DEVICE,
	}, {
		.virtual	= (unsigned long)S5P_VA_CMU_ACP,
		.pfn		= __phys_to_pfn(EXYNOS4270_PA_CMU_ACP),
		.length		= SZ_16K,
		.type		= MT_DEVICE,
	}, {
		.virtual	= (unsigned long)S5P_VA_SYSRAM,
		...
		.virtual	= (unsigned long)S5P_VA_SYSRAM_NS,
		.pfn		= __phys_to_pfn(EXYNOS4270_PA_SYSRAM_NS),
		.length		= SZ_4K,
		.type		= MT_DEVICE,
	}, {
		.virtual	= (unsigned long)S5P_VA_PPMU_RIGHT,
		.pfn		= __phys_to_pfn(EXYNOS4270_PA_PPMU_RIGHT),
		.length		= SZ_8K,
		.type		= MT_DEVICE,
	}, {
		.virtual	= (unsigned long)S5P_VA_PPMU_LEFT,
		.pfn		= __phys_to_pfn(EXYNOS4270_PA_PPMU_LEFT),
		.length		= SZ_8K,
		.type		= MT_DEVICE,
	}, {
...
static void __init exynos4_map_io(void)
{
	...
	if (soc_is_exynos4210()) {
		...
		iotable_init(exynos4270_iodesc, ARRAY_SIZE(exynos4270_iodesc));
	else
...
static void __init exynos4_init_clocks(int xtal)
{
	...
	else if (soc_is_exynos4270())
		exynos4270_register_clocks();
	else if (soc_is_exynos3470())
		exynos3470_register_clocks();
...
void __init exynos4_init_irq(void)
{
	...
	if (soc_is_exynos4212() || soc_is_exynos4270()) {
...
void __init exynos5_init_irq(void)
{
	...
	of_irq_init(exynos4_dt_irq_match);
#else
	gic_init(0, IRQ_PPI(0), (void __iomem *) S5P_VA_GIC_DIST, S5P_VA_GIC_CPU);
#endif
	gic_arch_extn.irq_set_wake = s3c_irq_wake;

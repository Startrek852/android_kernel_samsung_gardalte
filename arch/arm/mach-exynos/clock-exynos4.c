...
static unsigned long exynos4_fout_apll_get_rate(struct clk *clk)
{
	if (soc_is_exynos4210())
		return s5p_get_pll45xx(xtal_rate, __raw_readl(EXYNOS4_APLL_CON0),
					pll_4508);
	else if (soc_is_exynos4212() || soc_is_exynos4412() || soc_is_exynos4270() ||
					soc_is_exynos3470())
...
void __init clock_print(struct clksrc_clk *clk, int size)
{
	...
	for (; size > 0; size--, clk++) {
		...
		printk("%s: source is %s (%d), rate is %ld\n",
...
void __init_or_cpufreq exynos4_setup_clocks(void)
{
...
#if defined(CONFIG_SOC_EXYNOS4212) || defined(CONFIG_SOC_EXYNOS4412)
	pr_info("EXYNOS4: ACLK400=%ld ACLK266=%ld ACLK133=%ld\n",
			clk_get_rate(&exynos4212_clk_aclk_400_mcuisp.clk),
			clk_get_rate(&exynos4212_clk_aclk_266.clk),
			clk_get_rate(&exynos4_clk_aclk_133.clk));
#endif
#if defined(CONFIG_SOC_EXYNOS4270)
	pr_info("EXYNOS4: ACLK400=%ld ACLK266=%ld\n",
			clk_get_rate(&exynos4270_clk_aclk_400_mcuisp.clk),
			clk_get_rate(&exynos4270_clk_aclk_266.clk));
#endif
	...
	pr_info("EXYNOS4: ACLK400=%ld ACLK266=%ld\n",
			clk_get_rate(&exynos3470_clk_aclk_400_mcuisp.clk),
			clk_get_rate(&exynos3470_clk_aclk_266.clk));
#endif
...
#if defined(CONFIG_SOC_EXYNOS3470)
		if (clk_set_parent(&exynos4_clk_sclk_mmc0.clk,
				&exynos3470_clk_mout_mpll_user_top.clk))
			printk(KERN_ERR "Unable to set parent %s of clock %s.\n",
				exynos3470_clk_mout_mpll_user_top.clk.name,
				exynos4_clk_sclk_mmc2.clk.name);
		if (clk_set_parent(&exynos4_clk_sclk_mmc1.clk,
				&exynos3470_clk_mout_mpll_user_top.clk))
			printk(KERN_ERR "Unable to set parent %s of clock %s.\n",

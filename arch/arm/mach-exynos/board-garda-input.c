...
static int __init lcd_connect_check(char *str)
{
	mms_ts_pdata.lcd_connect = true;
	return 0;
}
early_param("tspconnect", lcd_connect_check);
#endif
...
void __init garda_tsp_init(u32 system_rev)
{
	...
	printk(KERN_ERR "%s touch : %d (%d)\n", __func__,
		i2c_devs_touch[0].irq, exynos3_get_revision());
#if defined(CONFIG_TOUCHSCREEN_ZINITIX_BT432)
	zinitix_power(0);
#endif
#if	defined(CONFIG_TOUCHSCREEN_ZINITIX_BT432) &&\

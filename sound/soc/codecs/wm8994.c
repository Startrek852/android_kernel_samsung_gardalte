...
static int configure_aif_clock(struct snd_soc_codec *codec, int aif)
{
	...
	if (rate >= 13500000) {
		...
		dev_info(codec->dev, "Dividing AIF%d clock to %dHz\n",
			aif + 1, rate);
	}
...
static void vmid_reference(struct snd_soc_codec *codec)
{
	...
	dev_info(codec->dev, "Referencing VMID, refcount is now %d\n",
		wm8994->vmid_refcount);
...
static bool wm8994_check_class_w_digital(struct snd_soc_codec *codec)
{
	...
	case WM8994_AIF2DACL_TO_DAC1L:
		dev_info(codec->dev, "Class W source AIF2DAC\n");
		source = 2 << WM8994_CP_DYN_SRC_SEL_SHIFT;
		break;
	case WM8994_AIF1DAC2L_TO_DAC1L:
		dev_info(codec->dev, "Class W source AIF1DAC2\n");
		source = 1 << WM8994_CP_DYN_SRC_SEL_SHIFT;
		break;
	case WM8994_AIF1DAC1L_TO_DAC1L:
		dev_info(codec->dev, "Class W source AIF1DAC1\n");
		source = 0 << WM8994_CP_DYN_SRC_SEL_SHIFT;
		break;
	default:
		dev_info(codec->dev, "DAC mixer setting: %x\n", reg);
		return false;
	}

	reg_r = snd_soc_read(codec, WM8994_DAC1_RIGHT_MIXER_ROUTING);
	if (reg_r != reg) {
		dev_info(codec->dev, "Left and right DAC mixers different\n");
		return false;
	}
...
{
	struct snd_soc_codec *codec = w->codec;
	dev_info(codec->dev, "SRC status: %x\n",
		snd_soc_read(codec,
			     WM8994_RATE_STATUS));
	return 0;
}
			  ...
			  unsigned int freq_in, unsigned int freq_out)
{
	...
	if (wm8994->fll_byp && src == WM8994_FLL_SRC_BCLK &&
		...
		dev_info(codec->dev, "Bypassing FLL%d\n", id + 1);
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	case WM8994_SYSCLK_MCLK1:
		...
		wm8994->mclk[0] = freq;
		dev_info(dai->dev, "AIF%d using MCLK1 at %uHz\n",
			dai->id, freq);
		break;
	...
	case WM8994_SYSCLK_MCLK2:
		...
		wm8994->mclk[1] = freq;
		dev_info(dai->dev, "AIF%d using MCLK2 at %uHz\n",
			dai->id, freq);
		break;

	case WM8994_SYSCLK_FLL1:
		wm8994->sysclk[dai->id - 1] = WM8994_SYSCLK_FLL1;
		dev_info(dai->dev, "AIF%d using FLL1\n", dai->id);
		break;

	case WM8994_SYSCLK_FLL2:
		wm8994->sysclk[dai->id - 1] = WM8994_SYSCLK_FLL2;
		dev_info(dai->dev, "AIF%d using FLL2\n", dai->id);
		break;
	...
	if (max(wm8994->aifclk[0], wm8994->aifclk[1]) < 50000 &&
	    !wm8994->aifdiv[0]) {
		dev_info(codec->dev, "Configuring AIFs for 128fs\n");
			    ...
			    struct snd_soc_dai *dai)
{
	...
	case 1:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK ||
			...
			dev_info(codec->dev, "AIF1 using split LRCLK\n");
		}
		break;
	case 2:
		aif1_reg = WM8994_AIF2_CONTROL_1;
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK ||
			...
			dev_info(codec->dev, "AIF2 using split LRCLK\n");
		}
		break;
	default:
		return -EINVAL;
	...
	if (pdata->max_channels_clocked[id] &&
		...
		dev_info(dai->dev, "Constraining channels to %d from %d\n",
	...
	dev_info(dai->dev, "Selected AIF%dCLK/fs = %d\n",
	...
	dev_info(dai->dev, "Using BCLK_DIV %d for actual BCLK %dHz\n",
	...
	dev_info(dai->dev, "Using LRCLK rate %d for actual LRCLK %dHz\n",
...
static void wm8958_button_det(struct snd_soc_codec *codec, u16 status)
{
	...
	pr_info("%s: status = 0x%x, report = 0x%x\n",
		__func__, status, report);

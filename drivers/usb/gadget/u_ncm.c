					...
					struct usb_configuration *c)
{
	...
	pr_debug("usb: %s MAC:%02X:%02X:%02X:%02X:%02X:%02X\n",
			__func__, ncm->ethaddr[0], ncm->ethaddr[1],
			ncm->ethaddr[2], ncm->ethaddr[3], ncm->ethaddr[4],
			ncm->ethaddr[5]);


	pr_debug("usb: %s before MAC:%02X:%02X:%02X:%02X:%02X:%02X\n",
			__func__, ncm->ethaddr[0], ncm->ethaddr[1],
			ncm->ethaddr[2], ncm->ethaddr[3], ncm->ethaddr[4],
			ncm->ethaddr[5]);
	...
	pr_debug("usb: %s after MAC:%02X:%02X:%02X:%02X:%02X:%02X\n",
			__func__, ncm->ethaddr[0], ncm->ethaddr[1],
			ncm->ethaddr[2], ncm->ethaddr[3], ncm->ethaddr[4],
			ncm->ethaddr[5]);
	if (ret) {
...
void set_ncm_device_descriptor(struct usb_device_descriptor *desc)
{
	...
	pr_debug("usb: %s idProduct=0x%x, DeviceClass=0x%x\n",
...
void set_ncm_ready(bool ready)
{
	if (ready != ncm_connect)
		pr_debug("usb: %s old status=%d, new status=%d\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_debug("usb: %s terminal_mode %s\n", __func__, buf);
	return ret;
}
...
static int create_terminal_attribute(struct device **pdev)
{
	...
	if (err) {
		pr_debug("usb: %s failed to create attr\n",
				__func__);
		return err;
	}
	return 0;
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) {
		...
		if (ctrl->bRequest == 0xf0) {
			...
			set_ncm_ready(true);
			pr_debug("usb: %s ver=0x%x vendor_id=0x%x\n",
	...
	if (value >= 0) {
		...
		if (rc < 0)
			pr_debug("usb: %s failed usb_ep_queue\n",
					__func__);
	}
	return value;
}

...
static struct dma_buf *vb2_ion_get_dmabuf(void *buf_priv)
{
	struct vb2_ion_buf *priv = buf_priv;
	struct dma_buf *dbuf;
	struct ion_handle *handle = priv->handle;
	struct ion_client *client = priv->ctx->client;

	if (WARN_ON(!priv->cookie.sgt))
		return NULL;

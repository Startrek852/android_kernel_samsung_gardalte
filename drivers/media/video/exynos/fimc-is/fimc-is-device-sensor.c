...
/*MIPI*/
#if defined(CONFIG_SOC_EXYNOS5250) || defined(CONFIG_SOC_EXYNOS5410) || defined(CONFIG_SOC_EXYNOS3470)
/* CSIS global control */
...
#define S5PCSIS_DPHYCTRL_HSS_MASK			(0x1f << 27) /* only for exynos3470 */
#define S5PCSIS_DPHYCTRL_ENABLE				(0x7 << 0) /* only for exynos3470 */
...
/* Interrupt mask. */
#define S5PCSIS_INTMSK					(0x10)
#define S5PCSIS_INTMSK_EN_ALL				(0xfff01037)
...
static void s5pcsis_enable_interrupts(unsigned long mipi_reg_base, bool on)
{
	...
	is_writel(0xFFF01037, mipi_reg_base + S5PCSIS_INTMSK); /* only for carmen */
}
...
static void s5pcsis_system_enable(unsigned long mipi_reg_base, int on)
{
	...
	if (on) {
		...
		val &= ~S5PCSIS_CTRL_ENABLE;
	is_writel(0x0000010D, mipi_reg_base + S5PCSIS_CTRL); /* only for carmen */
	...
	if (on)
		...
		val &= ~S5PCSIS_DPHYCTRL_ENABLE;
	is_writel(0x7000001F, mipi_reg_base + S5PCSIS_DPHYCTRL); /* only for carmen */
...
static void s5pcsis_set_hsync_settle(unsigned long mipi_reg_base, int settle)
{
	...
	val = (val & ~S5PCSIS_DPHYCTRL_HSS_MASK) | (0x6 << 28); /* only for exynos3470 */
				...
				struct fimc_is_frame_info *f_frame)
{
	...
	is_writel(val, mipi_reg_base + S5PCSIS_CONFIG);
#endif
	is_writel(0x000000AC, mipi_reg_base + S5PCSIS_CONFIG); /* only for carmen */

	__s5pcsis_set_format(mipi_reg_base, f_frame);
	s5pcsis_set_hsync_settle(mipi_reg_base, 6); /* only for carmen */
...
#if !defined(CONFIG_SOC_EXYNOS3470) /* only for carmen */
	...
	u32 flite_channel)
{
	...
	ext->sensor_con.peri_setting.i2c.channel = enum_sensor[SENSOR_NAME_S5K6A3].i2c_ch;
	...
	ext->sensor_con.product_name = 0;
	ext->sensor_con.peri_type = SE_I2C;
	ext->sensor_con.peri_setting.i2c.channel = enum_sensor[SENSOR_NAME_S5K3H5].i2c_ch;
	...
	/* S5K4H5 */
	enum_sensor[SENSOR_NAME_S5K4H5].sensor = SENSOR_NAME_S5K4H5;
	enum_sensor[SENSOR_NAME_S5K4H5].pixel_width = 3248 + 16;
	enum_sensor[SENSOR_NAME_S5K4H5].pixel_height = 2438 + 10;
	enum_sensor[SENSOR_NAME_S5K4H5].active_width = 3248;
	enum_sensor[SENSOR_NAME_S5K4H5].active_height = 2438;
	enum_sensor[SENSOR_NAME_S5K4H5].max_framerate = 30;
	enum_sensor[SENSOR_NAME_S5K4H5].csi_ch = 0;
	enum_sensor[SENSOR_NAME_S5K4H5].flite_ch = FLITE_ID_A;
	enum_sensor[SENSOR_NAME_S5K4H5].i2c_ch = 0;
	enum_sensor[SENSOR_NAME_S5K4H5].setfile_name =
			"setfile_4h5.bin";

	ext = &enum_sensor[SENSOR_NAME_S5K4H5].ext;
	memset(ext, 0x0, sizeof(struct sensor_open_extended));

	ext->sensor_con.product_name = 0;
	ext->sensor_con.peri_type = SE_I2C;
	ext->sensor_con.peri_setting.i2c.channel = enum_sensor[SENSOR_NAME_S5K4H5].i2c_ch;
	...
	ext->sensor_con.product_name = 0;
	ext->sensor_con.peri_type = SE_I2C;
	ext->sensor_con.peri_setting.i2c.channel = enum_sensor[SENSOR_NAME_S5K3H7].i2c_ch;
	ext->sensor_con.peri_setting.i2c.slave_address = 0x20;
	ext->sensor_con.peri_setting.i2c.speed = 400000;
	...
	ext->sensor_con.product_name = 0;
	ext->sensor_con.peri_type = SE_I2C;
	ext->sensor_con.peri_setting.i2c.channel = enum_sensor[SENSOR_NAME_IMX135].i2c_ch;

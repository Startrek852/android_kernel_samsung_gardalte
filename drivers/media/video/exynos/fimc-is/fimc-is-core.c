...
int fimc_is_runtime_suspend(struct device *dev)
{
	...
	if (core->pdata->clk_off) {
		ret = core->pdata->clk_off(core->pdev);
		if (ret) {
			err("fail to clk_off");
			goto p_err;
		}
	...
	if (core->pdata->sensor_power_off) {
		ret = core->pdata->sensor_power_off(core->pdev, 0);
		if (ret) {
			err("fail to sensor_power_off");
			goto p_err;
		}
	} else {
...
int fimc_is_runtime_resume(struct device *dev)
{
	...
	if (core->pdata->clk_cfg) {
		ret = core->pdata->clk_cfg(core->pdev);
		if (ret) {
			err("fail to clk_cfg");
			goto p_err;
		}
	...
	for (i = 0; i < FIMC_IS_MAX_NODES; i++) {
		...
		if (core->pdata->sensor_power_on) {
			ret = core->pdata->sensor_power_on(core->pdev, i);
			if (ret) {
				err("fail to sensor_power_on");
				goto p_err;
			}
		} else {
			err("failed to sensor_power_on\n");
	...
	if (core->pdata->clk_on) {
		ret = core->pdata->clk_on(core->pdev);
		if (ret) {
			err("fail to clk_on");
			goto p_err;
		}

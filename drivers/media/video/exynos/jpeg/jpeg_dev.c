...
static int jpeg_probe(struct platform_device *pdev)
{
	...
	dev = devm_kzalloc(&pdev->dev, sizeof(struct jpeg_dev), GFP_KERNEL);
	if (!dev) {
	...
	dev->reg_base = devm_request_and_ioremap(&pdev->dev, res);
...
#ifdef CONFIG_PM_RUNTIME
	if (soc_is_exynos4212() || soc_is_exynos4412() || soc_is_exynos4270() || soc_is_exynos3470())

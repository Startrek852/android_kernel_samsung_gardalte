...
static int hdmi_runtime_suspend(struct device *dev)
{
	...
	if (is_ip_ver_4c || is_ip_ver_5a)
		hdmiphy_set_power(hdev, 0);

	/* turn clocks off */
	clk_disable(res->sclk_hdmi);

	if (is_ip_ver_4p || is_ip_ver_5g)
...
static int hdmi_runtime_resume(struct device *dev)
{
	...
	if (is_ip_ver_4c)
		clk_enable(res->hdmiphy_apb);
	ret = hdmi_conf_apply(hdev);
	if (ret)
		goto fail;
	if (is_ip_ver_4c)
		clk_disable(res->hdmiphy_apb);
...
static void hdmi_resources_cleanup(struct hdmi_device *hdev)
{
	...
	/* put clocks */
	if (!IS_ERR_OR_NULL(res->hdmiphy_apb))
		clk_put(res->hdmiphy_apb);
...
static int hdmi_resources_init(struct hdmi_device *hdev)
{
	...
	res->hdmiphy_apb = clk_get(dev, "hdmiphy");
	if (IS_ERR_OR_NULL(res->hdmiphy_apb)) {
...
{
	u32 preset = HDMI_DEFAULT_PRESET;
	int ret;
	int i = 0;
	bool find = false;
	...
	if (state) {
		...
		for (i = 0; i < edid_presets_cnt; i++) {
			if (edid_presets[i].preset == preset &&
				edid_presets[i].supported == true) {
				find = true;
				break;
			}
		}
...
static int __devinit hdmi_probe(struct platform_device *pdev)
{
	...
	struct s5p_hdmi_platdata *pdata = dev->platform_data;
	struct resource *res;

...
static int sr130pc20_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (!state->initialized && ctrl->id != V4L2_CID_CAMERA_SENSOR_MODE
		...
		cam_warn("%s: WARNING, camera not initialized. ID = %d(0x%X)\n",
			__func__, ctrl->id & 0xFF, ctrl->id );
		return 0;
	}

	cam_dbg("%s: ID ==0x%08X, val = %d\n",
	...
	/*	if (RUNMODE_RUNNING == state->runmode) {
			...
			printk("fast capture mode (%s)\n",__func__);
		}*/
		if ((SENSOR_CAMERA == state->sensor_mode) && ctrl->value == true) {
			cam_dbg("Capture Mode!\n");
			state->format_mode = V4L2_PIX_FMT_MODE_CAPTURE;
			cam_info("(%s)format mode capture = %d ,%d\n",__func__,state->format_mode,V4L2_PIX_FMT_MODE_CAPTURE);
		} else
			state->format_mode = V4L2_PIX_FMT_MODE_PREVIEW;
		break;
...
{
	struct sr130pc20_state *state = to_state(sd);
	struct sr130pc20_platform_data *pdata = state->pdata;
	int err = -EINVAL;
	...
	if (val) {
		if (SR130PC20_HW_POWER_ON == pdata->power) {
			err = pdata->power_off(pdata);
			CHECK_ERR(err);
			msleep_debug(50, true);
		} else
			cam_err("reset: sensor is not powered\n");

		err = pdata->power_on(pdata);
		CHECK_ERR(err);
...
static int sr130pc20_s_power(struct v4l2_subdev *sd, int on)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sr130pc20_state *state = to_state(sd);
	 struct sr130pc20_platform_data *pdata = client->dev.platform_data;
	int tries = 3;
	int err = 0;
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_ACTIVE) {
		...
		if ((pdata->power == SR130PC20_HW_POWER_ON)
		    && (state->runmode != RUNMODE_CAPTURING))

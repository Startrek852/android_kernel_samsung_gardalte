...
/* #define ENABLE_FLASH_WITH_PREVIEW */
...
static const struct s5k4ecgx_regs regs_for_fw_version_1_1 = {
	...
	.effect = {
		S5K4ECGX_REGSET(V4L2_IMAGE_EFFECT_NONE, s5k4ecgx_Effect_Normal),
		S5K4ECGX_REGSET(V4L2_IMAGE_EFFECT_BNW, s5k4ecgx_Effect_Black_White),
		S5K4ECGX_REGSET(V4L2_IMAGE_EFFECT_SEPIA, s5k4ecgx_Effect_Sepia),
		S5K4ECGX_REGSET(V4L2_IMAGE_EFFECT_NEGATIVE,
...
static int s5k4ecgx_power(struct v4l2_subdev *sd, int flag)
{
   ...
   //xiebin.wang@20110530
	...
	if(flag == 1)
	{
		...
		udelay(30);
		regulator_enable(main_cam_sensor_a2v8_regulator);
		udelay(10);
		regulator_enable(main_cam_af_2v8_regulator);
		udelay(20);
#endif
		clk_enable(state->mclk);
		udelay(40);
#ifdef CONFIG_MACH_GARDA
		regulator_enable(main_cam_io_1v8_regulator);
		udelay(10);
		...
		// PCLK high
		s3c_gpio_cfgpin(GPIO_CAM_PCLK, S3C_GPIO_SFN(3));//PCLK
...
#endif
#ifdef CONFIG_MACH_GARDA
		regulator_disable(main_cam_io_1v8_regulator);
		udelay(10);
		regulator_disable(main_cam_sensor_a2v8_regulator);
		udelay(10);
		regulator_disable(main_cam_af_2v8_regulator);
		udelay(10);
...
static int s5k4ecgx_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if (s5k4ecgx_set_stored_parms(sd)) {
		dev_err(&client->dev,
			"%s: failed: set stored parameters\n",
			__func__);
		return -EIO;
	}
...
static int s5k4ecgx_set_capture_mode(struct v4l2_subdev *sd, int enable)
{
	struct s5k4ecgx_state *state =
		container_of(sd, struct s5k4ecgx_state, sd);
...
static int s5k4ecgx_start_capture(struct v4l2_subdev *sd)
{
	...
	if (parms->scene_mode != SCENE_MODE_NIGHTSHOT) {
		...
		case FLASH_MODE_ON:
			...
			if (pdata->flash_onoff(1))
				dev_err(&client->dev,
					"%s: Failed flash mode on.\n", __func__);
			break;
		default:
			break;
		}
	...
	if ((parms->scene_mode != SCENE_MODE_NIGHTSHOT) && (state->flash_on)) {
		...
		if (pdata->flash_onoff(0))
			dev_err(&client->dev,
				"%s: Failed flash mode off.\n", __func__);
...
#ifdef ENABLE_FLASH_WITH_PREVIEW
...
///	err |= s5k4ecgx_set_focus_mode(sd, stored_parms->focus_mode);
	...
	/*
	 * CAUTION: The frame rate (fps) is changed when setting preview size.
	 * When setting stored parameter, fps is must be set agian
	 * if there is the fps user wants. But, user consider fps can be
	 * supported at the preview size user sets.
	 */
...
static int s5k4ecgx_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAM_FLASH_MODE:
		...
		dev_err(&client->dev, "V4L2_CID_CAM_FLASH_MODE\n");
		break;
	...
	case V4L2_CID_WHITE_BALANCE_PRESET:
		...
		break;
	case V4L2_CID_IMAGE_EFFECT:
		dev_err(&client->dev, "V4L2_CID_IMAGE_EFFECT\n");
		if (state->runmode == S5K4ECGX_RUNMODE_RUNNING) {
	...
	case V4L2_CID_FOCUS_MODE:
		...
		/* TODO: V4L2_CID_FOCUS_MODE is not supported now. */
		err = 0;
	...
	case V4L2_CID_CAM_SINGLE_AUTO_FOCUS:
		dev_err(&client->dev, "V4L2_CID_CAM_SINGLE_AUTO_FOCUS\n");
		err = s5k4ecgx_start_auto_focus(sd);
		if (err < 0) {
			dev_err(&client->dev,
				"%s: failed single auto focus %d\n",
				__func__, value);
		}
		break;
	...
	case V4L2_CID_CAM_FRAME_RATE:
		...
		/*
		 * Fps is not controlled by timeperframe.
		 * It is controlled by setting fps register to sensor.
		 *
		 * If fps is controlled by timeperframe,
		...
		if (state->runmode == S5K4ECGX_RUNMODE_RUNNING) {
			...
			dev_err(&client->dev,
				"%s: trying to set frame rate when not "
				"in preview modei(%d)\n",
				__func__, value);
			stored_parms->fps = value;
			err = 0;
		}
		break;
	case V4L2_CID_CAPTURE:
		dev_err(&client->dev, "V4L2_CID_CAPTURE\n");
		err = s5k4ecgx_set_capture_mode(sd, value);
		break;
...
static int s5k4ecgx_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		if (state->oprmode == S5K4ECGX_OPRMODE_VIDEO) {
			if (s5k4ecgx_set_preview_start(sd)) {
				dev_err(&client->dev,
					"%s: Failed preview start.\n", __func__);
				return -EIO;
			}
		} else {
#ifdef ENABLE_FLASH_WITH_PREVIEW
			...
			if (pdata->flash_onoff(0))
				dev_err(&client->dev,
					"%s: Failed flash mode off.\n", __func__);
#else
			if (s5k4ecgx_start_capture(sd)) {
				dev_err(&client->dev,
					"%s: Failed capture start.\n", __func__);
				return -EIO;
			}
#endif
		}
		...
		if (s5k4ecgx_set_preview_stop(sd) < 0) {
...
static int s5k4ecgx_s_power(struct v4l2_subdev *sd, int on)
{
	...
	dev_dbg(&client->dev, "%s: %d\n", __func__, __LINE__);
        if (on) {
		ret = s5k4ecgx_power(sd, S5K4ECGX_HW_POWER_ON);

		s5k4ecgx_init_parameters(sd);

		if (s5k4ecgx_init(sd, on)) {
			dev_err(&client->dev,
				"%s: Failed preview start.\n", __func__);
			return -EIO;
		}

		if (s5k4ecgx_init_formats(sd, NULL)) {
			dev_err(&client->dev,
				"%s: Failed init formats.\n", __func__);
			return -EIO;
		}

		state->power_on = S5K4ECGX_HW_POWER_ON;
	} else {
		ret = s5k4ecgx_power(sd, S5K4ECGX_HW_POWER_OFF);
...
#else
	enum s5k4ecgx_oprmode stype = state->oprmode;
#endif
			...
			const struct i2c_device_id *id)
{
	...
	if (gpio_request(GPIO_CAM_FLASH_EN, "GPM3_4") < 0) {
		pr_err("cam_cfg_init: fail to get gpio(FLASH_EN),"
			" err=%d\n", ret);
		goto err_gpio;
	}

	if (gpio_request(GPIO_CAM_FLASH_SET, "GPM3_6") < 0) {
		pr_err("cam_cfg_init: fail to get gpio(FLASH_SET),"
			" err=%d\n", ret);
		goto err_gpio;
	}
#endif

...
void fimc_hw_reset(struct fimc_dev *dev)
{
	...
	cfg = readl(dev->regs + FIMC_REG_CISTATUS);
	cfg &= ~FIMC_REG_CISTATUS_LASTCAPT_END;
	writel(cfg, dev->regs + FIMC_REG_CISTATUS);
...
void fimc_hw_set_target_format(struct fimc_ctx *ctx)
{
	...
	case FIMC_FMT_RGB444...FIMC_FMT_RGB888:
		...
		break;
	case FIMC_FMT_YCBCR420_2P...FIMC_FMT_YCRCB420_3P:
		cfg |= FIMC_REG_CITRGFMT_YCBCR420;
		break;
	case FIMC_FMT_YCBYCR422_1P...FIMC_FMT_YCRCB422_2P:
...
void fimc_hw_set_in_dma(struct fimc_ctx *ctx)
{
	...
	case FIMC_FMT_YCBYCR422_1P...FIMC_FMT_YCRCB422_2P:
		...
		break;
	default:
		pr_err("unsupported input dma format\n");
		break;
	}
...
void fimc_hw_set_enable_lastend(struct fimc_dev *dev)
{
	u32 cfg = readl(dev->regs + FIMC_REG_CIOCTRL);
			      ...
			      struct s5p_fimc_isp_info *cam)
{
	...
	if (cam->bus_type == FIMC_ITU_601 || cam->bus_type == FIMC_ITU_656) {
		...
		if (fimc_fmt_is_user_defined(f->fmt->color))
			cfg |= FIMC_REG_CISRCFMT_ITU601_8BIT;
	} else if (cam->bus_type == FIMC_IS_WB) {
		cfg |= FIMC_REG_CISRCFMT_ITU601_8BIT;
	}
			    ...
			    struct s5p_fimc_isp_info *cam)
{
	...
	case FIMC_MIPI_CSI2:
		...
		case V4L2_MBUS_FMT_INTERLEAVED:
...
void fimc_hw_enable_frame_end_irq(struct fimc_dev *dev, bool on)
{
	u32 cfg = readl(dev->regs + FIMC_REG_CIGCTRL);
	if (on)
...
void fimc_activate_capture(struct fimc_ctx *ctx)
{
	...
	fimc_hw_enable_frame_end_irq(ctx->fimc_dev, false);
}

void fimc_deactivate_capture(struct fimc_dev *fimc)
{
	fimc_hw_dis_capture(fimc);
	fimc_hw_enable_frame_end_irq(fimc, true);
	fimc_hw_enable_scaler(fimc, false);
}

void fimc_hwset_sysreg_camblk_isp_wb(struct fimc_dev *fimc)
{
	u32 camblk_cfg = readl(SYSREG_CAMERA_BLK);
	u32 ispblk_cfg = readl(SYSREG_ISP_BLK);

	if (((camblk_cfg & 0x00700000) >> 20) == 0x3) {

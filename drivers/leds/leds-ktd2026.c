...
static void ktd2026_led_brightness_work(struct work_struct *work)
{
		...
		if (led->brightness==0)ktd2026_leds_on(led->channel, LED_EN_OFF, 0);
			...
			u8 ledcc)
{
	...
	{
		if (slopemode)
			data->shadow_reg[KTD2026_REG_LED_EN] |= LED_EN_PWM1 << led;
...
void ktd2026_set_timerslot_control(int timer_slot)
{
	...
	data->shadow_reg[KTD2026_REG_EN_RST] |= timer_slot << CNT_TIMER_SLOT_SHIFT;
}
					...
					u8 brightness)
{
	...
	ktd2026_set_trise_tfall(0, 0, 0);
	printk(KERN_CRIT "on_time=%d, off_time=%d, period=%d, duty=%d\n", on_time, off_time, (on_time+off_time) * 4 + 2, on_time*255 / (on_time + off_time) );
}

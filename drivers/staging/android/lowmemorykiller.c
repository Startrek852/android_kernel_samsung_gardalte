...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	int all_selected_oom_score = 0;
	int max_selected_oom_score_idx = 0;
#else
	...
	for_each_process(tsk) {
		...
		if (tasksize <= 0)
			continue;
#ifdef ENHANCED_LMK_ROUTINE
		if (all_selected_oom_score < LOWMEM_DEATHPENDING_DEPTH)
			for (i = 0; i < LOWMEM_DEATHPENDING_DEPTH; i++) {
				if (!selected[i]) {
					is_exist_oom_task = 1;
					max_selected_oom_score_idx = i;
					break;
				}
			}
		else if (selected_oom_score_adj[max_selected_oom_score_idx]
							< oom_score_adj ||
			(selected_oom_score_adj[max_selected_oom_score_idx]
							== oom_score_adj &&
			selected_tasksize[max_selected_oom_score_idx]
							< tasksize))
			is_exist_oom_task = 1;

		if (is_exist_oom_task) {
			selected[max_selected_oom_score_idx] = p;
			selected_tasksize[max_selected_oom_score_idx] = tasksize;
			selected_oom_score_adj[max_selected_oom_score_idx] = oom_score_adj;

			if (all_selected_oom_score < LOWMEM_DEATHPENDING_DEPTH)
				all_selected_oom_score++;

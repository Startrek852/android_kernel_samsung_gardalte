...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		if (slot->host->pdata->cd_type == DW_MCI_CD_GPIO)
			slot->host->pdata->cfg_gpio(4);
...
static void dw_mci_enable_sdio_irq(struct mmc_host *mmc, int enb)
{
	...
	if (enb) {
		...
		if ((host->quirks & DW_MMC_QUIRK_SDIO_IRQ_EXT_GPIO_POLL) &&
				!gpio_get_value(host->pdata->ext_irq_gpio))
		{
			mci_writel(host, INTMASK,
				   (int_mask & ~SDMMC_INT_SDIO(slot->id)));
...
static int dw_mci_1_8v_signal_voltage_switch(struct dw_mci_slot *slot)
{
	...
	dev_warn(&host->dev, "card still busy state\n");
	return -ENOMEDIUM;

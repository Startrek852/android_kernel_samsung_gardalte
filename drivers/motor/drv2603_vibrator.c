...
static int __devinit drv2603_vibrator_probe(struct platform_device *pdev)
{
	...
	ret = platform_device_register(&s3c_device_timer[pdata->pwm_id]);
	if (ret) {
		printk(KERN_ERR
			"[VIB] failed to register pwm timer for vibrator: %d\n", ret);
		goto err_pwm_reg;
	}
...
err_to_dev_reg:
err_request_clk:
	platform_device_unregister(&s3c_device_timer[pdata->pwm_id]);
err_pwm_reg:
	kfree(ddata);
err_free_mem:
	return ret;

...
static void sec_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	unsigned	i;
	...
	for (i = 0; i < chip->ngpio; i++, gpio++) {
		...
		cfg = (__raw_readl(ourchip->base) >> (i*4))& 0x0f;
		is_hi = (__raw_readl(ourchip->base+0x04) >> i)& 0x01;
		pud = (__raw_readl(ourchip->base+0x08) >> i*2)& 0x03;
		drv = (__raw_readl(ourchip->base+0x0C) >> i*2)& 0x03;
		seq_printf(s, " gpio-%-3d (%-20.20s) %s %s CON:0x%1x DAT:0x%1x PUD:0x%1x DRV:0x%1x ",
			gpio, label ?: "(none)",
			cfg == 0 ? "in " : (cfg == 1 ? "out" : "alt"),
...
#if defined(CONFIG_SOC_EXYNOS4412) || defined(CONFIG_SOC_EXYNOS4212) || defined(CONFIG_SOC_EXYNOS4270) || defined(CONFIG_SOC_EXYNOS3470)
	{
		.chip	= {
			.base	= EXYNOS4_GPA0(0),
			.ngpio	= EXYNOS4_GPIO_A0_NR,
...
#endif /* CONFIG_SOC_EXYNOS4412 || CONFIG_SOC_EXYNOS4412 || CONFIG_SOC_EXYNOS4270 || CONFIG_SOC_EXYNOS3470 */
};

#if defined(CONFIG_SOC_EXYNOS3470)
static struct samsung_gpio_pm samsung_gpio_pm_nop = { NULL, NULL };
#endif
...
#if defined(CONFIG_SOC_EXYNOS3470)
		/* do not restore eMMC's GPIOs */
		.pm	= &samsung_gpio_pm_nop,
#endif
	}, {
		.chip	= {
			.base	= EXYNOS4_GPK1(0),
...
static __init int samsung_gpiolib_init(void)
{
	...
	void __iomem *gpf_base, *gpj_base, *gpm_base, *gpx_base;
#if defined(CONFIG_SOC_EXYNOS4412) || defined(CONFIG_SOC_EXYNOS4212)
...
#endif
	} else if (soc_is_exynos4412() || soc_is_exynos4212() || soc_is_exynos4270() || soc_is_exynos3470()) {
		/* gpio part1 */
		gpio_base1 = ioremap(EXYNOS4_PA_GPIO1, SZ_4K);
		if (gpio_base1 == NULL) {
			pr_err("unable to ioremap for gpio_base1\n");

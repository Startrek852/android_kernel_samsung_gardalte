...
static inline u32 s3c_gpio_do_getLH(struct samsung_gpio_chip *chip,
					  unsigned int off)
{
...
unsigned samsung_gpio_getcfg_4bit(struct samsung_gpio_chip *chip,
 				      unsigned int off)
{
...
static unsigned s3c_gpio_getcfg_dvs(unsigned int pin)
{
	...
	if (chip) {
		chip->config->get_config = samsung_gpio_getcfg_4bit;
		offset = pin - chip->chip.base;
...
static samsung_gpio_pull_t s3c_gpio_get_slp_cfgpin(unsigned int pin)
{
	struct samsung_gpio_chip *chip = samsung_gpiolib_getchip(pin);
	void __iomem *reg;
	unsigned long flags;
	...
	return (__force samsung_gpio_pull_t)con;
}
...
static samsung_gpio_pull_t s3c_gpio_slp_getpull_updown(unsigned int pin)
{
	struct samsung_gpio_chip *chip = samsung_gpiolib_getchip(pin);
	void __iomem *reg;
	unsigned long flags;

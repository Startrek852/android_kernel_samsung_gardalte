...
#else
#define exynos4270_set_drex(target_freq) do { } while (0)
#endif
...
static struct devfreq_simple_ondemand_data exynos4270_mif_governor_data = {
	.pm_qos_class	= PM_QOS_BUS_THROUGHPUT,
	.upthreshold	= 30,
};
#endif
...
#if defined(CONFIG_DEVFREQ_GOV_SIMPLE_ONDEMAND)
	data->devfreq = devfreq_add_device(dev, &exynos4270_mif_devfreq_profile,
					&devfreq_simple_ondemand, &exynos4270_mif_governor_data);
#endif
...
static int exynos4270_busfreq_mif_suspend(struct device *dev)
{
	...
	unsigned long suspend_freq = 200000;
...
static int exynos4270_busfreq_mif_resume(struct device *dev)
{
	if (pm_qos_request_active(&exynos4270_mif_qos))
		pm_qos_update_request(&exynos4270_mif_qos, 200000);
	return 0;
}

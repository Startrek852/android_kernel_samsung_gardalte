				...
				bool enable)
{
	...
	if (enable && (battery->siop_level < 100)) {
		val.intval = battery->siop_level;
		psy_do_property("sec-charger", set,
...
static bool sec_bat_voltage_check(struct sec_battery_info *battery)
{
	...
	if (sec_bat_check_recharge(battery)) {
		...
		dev_info(battery->dev,
			"%s: NONE -> 2ND %d\n", __func__, battery->charging_mode);
		sec_bat_set_charge(battery, true);
		return false;
	}
...
#else
static void sec_bat_event_expired_work(struct work_struct *work)
{
	struct sec_battery_info *battery = container_of(
		work, struct sec_battery_info, event_termination_work.work);
...
#else
	cancel_delayed_work(&battery->event_termination_work);
#endif
	battery->event &= (~battery->event_wait);
...
#else
		schedule_delayed_work(&battery->event_termination_work,
			battery->pdata->event_waiting_time * HZ);
#endif
		dev_dbg(battery->dev,
					...
					struct sec_battery_info *battery)
{
	...
	if (battery->pdata->full_condition_skip_check) {
...
static int battery_get_curr_avg(
				struct sec_battery_info *battery)
{
	int curr_avg;
	pr_debug("%s\n", __func__);

	/* if 0% && under 3.4v && low power charging, power off */
	if ((battery->capacity <= 0) &&
		(battery->voltage_now < 3400) &&
		(battery->voltage_now < battery->voltage_ocv) &&
		((battery->current_now <
		battery->pdata->charging_current[
		POWER_SUPPLY_TYPE_MAINS].input_current_limit) ||
		((battery->health == POWER_SUPPLY_HEALTH_OVERHEAT) ||
		(battery->health == POWER_SUPPLY_HEALTH_COLD)))) {
			pr_debug("%s: SOC(%d), Vnow(%d), Vocv(%d), Inow(%d)\n",
				__func__, battery->capacity,
				battery->voltage_now,
				battery->voltage_ocv,
				battery->current_now);
...
continue_monitor:
	dev_dbg(battery->dev,
		"%s: Status(%s), Health(%s), Cable(%d), siop_level(%d)\n",
		__func__,
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
		if (battery->factory_mode || battery->pdata->check_jig_status())
			dev_dbg(battery->dev, "%s: No check in factory mode\n",
			__func__);
		else
...
#else
	INIT_DELAYED_WORK_DEFERRABLE(&battery->event_termination_work,
		sec_bat_event_expired_work);
#endif

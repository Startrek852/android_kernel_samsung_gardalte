...
{
#if defined(CONFIG_LCD_MIPI_TC358764_LVDS)
...
/*		dev_info(dsim->dev, "[DSIM:INFO] : %s clear rx fifo : %08x\n", __func__, rx_fifo); */
	...
	if (i > 0)
		dev_info(dsim->dev, "[DSIM:INFO] : %s clear rx fifo : %08x (cnt %d/%d)\n",
			__func__, rx_fifo, i, DSIM_MAX_RX_FIFO);
	mutex_unlock(&dsim_rd_wr_mutex);
	return 0;
	...
	unsigned int scaler)
{
	...
	if (soc_is_exynos5250() || soc_is_exynos4270() || soc_is_exynos3470()) {
...
int s5p_mipi_dsi_simple_link(struct mipi_dsim_device *dsim)
{
	dsim->state = DSIM_STATE_STOP;
...
#if defined(GPIO_LCD_ESD_DETECT) && !defined(CONFIG_FB_I80_COMMAND_MODE)
...
#if 0
#if defined(GPIO_LCD_ESD_DETECT) && !defined(CONFIG_FB_I80_COMMAND_MODE)
	/* this is non-continuous mode for video-mode, Make LP11 in every Vsync */
	if (int_src & TX_FRAME_DONE) {
		udelay(21);
...
#if defined(CONFIG_LCD_MIPI_TC358764) || defined(CONFIG_LCD_MIPI_TC358764_LVDS)
	s5p_mipi_dsi_enable_hs_clock(dsim, 1);
...
static int s5p_mipi_dsi_enable(struct mipi_dsim_device *dsim)
{
...
#if !defined(CONFIG_LCD_MIPI_NT35510)&&!defined(CONFIG_LCD_MIPI_NT35596)&&!defined(CONFIG_LCD_MIPI_S6E63M0)&&!defined(CONFIG_LCD_MIPI_S6E8AA4)
...
#if defined(CONFIG_LCD_MIPI_NT35510) || defined(CONFIG_LCD_MIPI_NT35596) || defined(CONFIG_LCD_MIPI_HX8369B) || defined(CONFIG_LCD_MIPI_S6E63M0) ||defined(CONFIG_LCD_MIPI_S6E8AA4)
	...
	/* enable interrupts */
	int_msk = SFR_PL_FIFO_EMPTY | RX_DAT_DONE | TX_FRAME_DONE;
	s5p_mipi_dsi_set_interrupt_mask(dsim, int_msk, 0);
...
#if !defined(CONFIG_LCD_MIPI_TC358764_LVDS) || !defined(CONFIG_LCD_MIPI_NT35510)
	dsim->dsim_lcd_drv->suspend(dsim);
#endif
	/* disable interrupts */
	int_msk = SFR_PL_FIFO_EMPTY | RX_DAT_DONE | TX_FRAME_DONE;
...
static int s5p_mipi_dsi_probe(struct platform_device *pdev)
{
	...
	if (dsim_pd->bootlogo == true) {
		s5p_mipi_dsi_simple_link(dsim);
	} else {
		s5p_mipi_dsi_init_dsim(dsim);
		s5p_mipi_dsi_init_link(dsim);
	}

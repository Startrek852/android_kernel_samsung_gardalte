...
struct s3c_fb {
	...
	struct clk		*bus_mdnie0;
	struct clk		*sclk_mdnie0;
	struct clk		*mdnie0;
	void __iomem		*regs;
	struct s3c_fb_variant	 variant;
	struct mutex			lock;
...
{
#if defined(CONFIG_FB_I80_COMMAND_MODE)
	int clkdiv = s3c_fb_calc_pixclk(sfb, win_mode->pixclock * 2);
#else
...
#if defined(CONFIG_LCD_MIPI_HX8369B) ||defined(CONFIG_LCD_MIPI_NT35596)||defined(CONFIG_LCD_MIPI_S6E63M0)||defined(CONFIG_LCD_MIPI_S6E8AA4)  /* Mipi-Signal RGB order */
...
#if defined(CONFIG_FB_I80_COMMAND_MODE)
static void s3c_fb_trigger(struct s3c_fb *sfb)
{
	u32 irq_sts_reg;
...
static int s3c_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	dsim_device = pd->dsim0_device;

	if (!pd->dsim_pd->lcd_connected)
	return ret;
	...
	case FB_BLANK_UNBLANK:
		...
		if (pd->dsim_displayon)
			pd->dsim_displayon(dsim_device);
...
static void garda_clear_te_gpio_int(void)
{
	int reg;
	reg = __raw_readl(S5P_EINT_PEND(0));
	__raw_writel(reg & 0x8, S5P_EINT_PEND(0));
}
...
static void s3c_fb_activate_vsync(struct s3c_fb *sfb)
{
	...
	struct s3c_fb_platdata *pd = sfb->pdata;
	struct device *dsim_device = pd->dsim0_device;
	...
	if (!prev_refcount) {
		garda_clear_te_gpio_int();
		s3c_fb_enable_irq(sfb);
		// dev_info(sfb->dev, "s3c_fb_enable_irq\n");
	}

	dsim->pd->fb_activate_vsync++;
...
static irqreturn_t s3c_fb_irq(int irq, void *dev_id)
{
	...
	if (sfb->vsync_info.irq_refcount) {
		...
		if (sfb->vsync_info.draw_request) {
			irq_sts_reg = readl(sfb->regs + TRIGCON);
			irq_sts_reg |= SWTRGCMD_I80_RGB | TRGMODE_I80_RGB;
			writel(irq_sts_reg, sfb->regs + TRIGCON);
...
void dsim_hs_clock_reset(struct s3c_fb *sfb)
{
	struct s3c_fb_platdata *pd = sfb->pdata;
	struct device *dsim_device = pd->dsim0_device;

	pd->dsim_hs_ctrl(dsim_device, 0);
	ndelay(700);
	pd->dsim_hs_ctrl(dsim_device, 1);
	udelay(1);
}
...
static void __devinit s3c_fb_missing_pixclock(struct fb_videomode *mode)
{
	...
	div *= mode->refresh ? : 40;
#elif defined(CONFIG_LCD_MIPI_HX8369B)
	/* it is not same as expression of framebuffer.cpp,
	 * so I add compensation value for CTS testRefreshRate */
	div *= get_fps() - 2;
#else
	div *= mode->refresh ? : 60;
#endif
...
static int __devinit s3c_fb_probe(struct platform_device *pdev)
{
	...
	sfb->bus_mdnie0 = clk_get(NULL, "apb_mdnie0");
	if (IS_ERR(sfb->bus_mdnie0)) {
		dev_err(dev, "failed to get bus clock\n");
		ret = PTR_ERR(sfb->bus_mdnie0);
		goto err_sfb;
	}

#ifndef CONFIG_PM_RUNTIME
	clk_enable(sfb->bus_mdnie0);
#endif
	sfb->sclk_mdnie0 = clk_get(NULL, "sclk_mdnie0");
	if (IS_ERR(sfb->sclk_mdnie0)) {
		dev_err(dev, "failed to get sclk clock\n");
		ret = PTR_ERR(sfb->sclk_mdnie0);
		goto err_sfb;
	}
#ifndef CONFIG_PM_RUNTIME
	clk_enable(sfb->sclk_mdnie0);
#endif

	sfb->mdnie0 = clk_get(NULL, "mdnie0");
	if (IS_ERR(sfb->mdnie0)) {
		dev_err(dev, "failed to get ip clock\n");
		ret = PTR_ERR(sfb->mdnie0);
		goto err_sfb;
	}
#ifndef CONFIG_PM_RUNTIME
	clk_enable(sfb->mdnie0);
#endif
#endif
...
static int s3c_fb_disable(struct s3c_fb *sfb)
{
	...
	if (sfb->pdata->backlight_ctrl)
		sfb->pdata->backlight_ctrl(0);

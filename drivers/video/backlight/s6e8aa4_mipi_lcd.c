...
static const unsigned char SEQ_GAMMA_CONDITION_SET_KMINI[] = {
	0xCA,
...
static void init_lcd(struct mipi_dsim_device *dsim)
{
	while (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
		(unsigned int)SEQ_APPLY_LEVEL_2_KEY_ENABLE,
			ARRAY_SIZE(SEQ_APPLY_LEVEL_2_KEY_ENABLE)) == -1)
		dev_err(dsim->dev, "fail to send apply_level_2_key command.\n");

	while (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
		(unsigned int)SEQ_APPLY_MTP_KEY_ENABLE,
			ARRAY_SIZE(SEQ_APPLY_MTP_KEY_ENABLE)) == -1)
		dev_err(dsim->dev, "fail to send apply_mtp_key command.\n");
	...
	while (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(unsigned int)SEQ_GAMMA_CONDITION_SET_KMINI,
				ARRAY_SIZE(SEQ_GAMMA_CONDITION_SET_KMINI)) == -1)
		dev_err(dsim->dev, "fail to send gamma_condition_set command.\n");

	while (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(unsigned int)SEQ_GAMMA_UPDATE,
				ARRAY_SIZE(SEQ_GAMMA_UPDATE)) == -1)
		dev_err(dsim->dev, "fail to send gamma_update command.\n");
	...
	while (s5p_mipi_dsi_wr_data(dsim, MIPI_DSI_DCS_LONG_WRITE,
			(unsigned int)SEQ_POWER_CONTROL_F4,
				ARRAY_SIZE(SEQ_POWER_CONTROL_F4)) == -1)

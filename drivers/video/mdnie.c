...
static int mdnie_send_sequence(struct mdnie_info *mdnie, const unsigned short *seq)
{
	...
	/* when booting, register 0x08 if mdnie is not valid. so update Forcely */
	if( is_first ) {
		mdnie_write( 0x08, 0x03 );
		is_first = FALSE;
	}
...
#if !defined(CONFIG_FB_MDNIE_PWM) && defined(CONFIG_LCD_CABC)

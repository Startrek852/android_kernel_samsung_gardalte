...
static void __iomem *exynos4_base_drexI_1;
static void __iomem *exynos4_base_drexI_0;
#endif
...
static int exynos_register_thermal(struct thermal_sensor_conf *sensor_conf)
{
	...
	th_zone->therm_dev = thermal_zone_device_register(sensor_conf->name,
			EXYNOS_ZONE_COUNT, NULL, &exynos_dev_ops, 1, 1, PASSIVE_INTERVAL,
			IDLE_INTERVAL);
...
#else
		case TMU_THR_LV1:
			__raw_writel(AREF_NORMAL,(exynos4_base_drexI_0 + DREX_TIMINGAREF));
			__raw_writel(AREF_NORMAL,(exynos4_base_drexI_1 + DREX_TIMINGAREF));
			break;
		case TMU_THR_LV2:
			__raw_writel(AREF_HOT,(exynos4_base_drexI_0 + DREX_TIMINGAREF));
			__raw_writel(AREF_HOT,(exynos4_base_drexI_1 + DREX_TIMINGAREF));
			break;
		case TMU_THR_LV3:
			__raw_writel(AREF_CRITICAL,(exynos4_base_drexI_0 + DREX_TIMINGAREF));
			__raw_writel(AREF_CRITICAL,(exynos4_base_drexI_1 + DREX_TIMINGAREF));
			break;
#endif
	}
}
...
static int __devinit exynos_tmu_probe(struct platform_device *pdev)
{
	...
	exynos4_base_drexI_0 = ioremap(EXYNOS4_PA_DMC0_4X12, SZ_64K);
	exynos4_base_drexI_1 = ioremap(EXYNOS4_PA_DMC1_4X12, SZ_64K);
	if(!exynos4_base_drexI_0 || !exynos4_base_drexI_1) {
		ret = -ENOMEM;
...
#ifndef CONFIG_ARM_TRUSTZONE
	iounmap(exynos4_base_drexI_0);
	iounmap(exynos4_base_drexI_1);
#endif
	platform_set_drvdata(pdev, NULL);

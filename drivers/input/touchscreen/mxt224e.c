...
struct mxt224e_data {
	...
	const struct mxt224e_platform_data *pdata;
#ifdef CONFIG_HAS_EARLYSUSPEND
...
struct mxt_fw_info {
	...
	struct mxt224e_data *data;
};

#define CLEAR_MEDIAN_FILTER_ERROR
struct mxt224e_data *copy_data;
...
static int read_mem(struct mxt224e_data *data, u16 reg, u8 len, u8 * buf)
{
	int ret;
...
static int write_mem(struct mxt224e_data *data, u16 reg, u8 len, const u8 * buf)
{
	int ret;
	u8 tmp[len + 2];
...
static int __devinit mxt224_reset(struct mxt224e_data *data)
{
	u8 buf = 1u;
	return write_mem(data, data->cmd_proc + CMD_RESET_OFFSET, 1, &buf);
}

static int __devinit mxt224_backup(struct mxt224e_data *data)
{
	u8 buf = 0x55u;
	return write_mem(data, data->cmd_proc + CMD_BACKUP_OFFSET, 1, &buf);
}

static int get_object_info(struct mxt224e_data *data, u8 object_type, u16 *size,
			   u16 *address)
{
	int i;
...
static int write_config(struct mxt224e_data *data, u8 type, const u8 * cfg)
{
	int ret;
	u16 address;
	u16 size;
...
static int mxt_read_object(struct mxt224e_data *data,
...
static int mxt_write_object(struct mxt224e_data *data,
				 u8 type, u8 offset, u8 val)
{
	int ret;
	u16 address;
...
static int __devinit calculate_infoblock_crc(struct mxt224e_data *data,
					     u32 *crc_pointer)
{
	u32 crc = 0;
...
uint8_t calibrate_chip(struct mxt224e_data *data)
{
	u8 cal_data = 1;
	int ret = 0;
...
static void mxt224_ta_cb(struct tsp_callbacks *cb, bool ta_status)
{
	struct mxt224e_data *data =
			container_of(cb, struct mxt224e_data, callbacks);
...
void check_chip_calibration(struct mxt224e_data *data,
...
static int __devinit mxt224_init_touch_driver(struct mxt224e_data *data)
{
...
{
	struct mxt224e_data *data = container_of(work,
				struct mxt224e_data, work_dvfs_chg.work);


	mutex_lock(&data->dvfs_lock);

	if (TSP_BOOSTER_LEVEL1 == data->boost_level) {
		dev_dbg(&data->client->dev, "%s & Off all\n", __func__);

		if (pm_qos_request_active(&data->tsp_cpu_qos))
			pm_qos_remove_request(&data->tsp_cpu_qos);

		if (pm_qos_request_active(&data->tsp_mif_qos))
			pm_qos_remove_request(&data->tsp_mif_qos);

		if (pm_qos_request_active(&data->tsp_int_qos))
			pm_qos_remove_request(&data->tsp_int_qos);
	} else {
		if (pm_qos_request_active(&data->tsp_cpu_qos))
			pm_qos_update_request(&data->tsp_cpu_qos, 800000); /* CPU 800MHz */

		if (pm_qos_request_active(&data->tsp_mif_qos))
			pm_qos_update_request(&data->tsp_mif_qos, 267000); /* MIF 267MHz */

		if (pm_qos_request_active(&data->tsp_int_qos))
			pm_qos_update_request(&data->tsp_int_qos, 160000); /* INT 160MHz */

		dev_dbg(&data->client->dev, "%s called!\n", __func__);
	}
...
{
	struct mxt224e_data *data = container_of(work,
				struct mxt224e_data, work_dvfs_off.work);
	...
	dev_dbg(&data->client->dev, "TSP DVFS Off %d\n", data->boost_level);
}

static void set_dvfs_lock(struct mxt224e_data *data, uint32_t on, bool mode)
{
	if (TSP_BOOSTER_DISABLE == data->boost_level)
		return;
	...
	if (on == 0) {
		...
		if (!data->dvfs_lock_status || mode) {
			if (data->dvfs_lock_status && mode) {
				cancel_delayed_work(&data->work_dvfs_chg);

				if (pm_qos_request_active(&data->tsp_cpu_qos))
					pm_qos_update_request(&data->tsp_cpu_qos, 1000000);
				else
					pm_qos_add_request(&data->tsp_cpu_qos, PM_QOS_CPU_FREQ_MIN, 1000000);

				if (pm_qos_request_active(&data->tsp_mif_qos))
					pm_qos_update_request(&data->tsp_mif_qos, 400000);
				else
					pm_qos_add_request(&data->tsp_mif_qos, PM_QOS_BUS_THROUGHPUT, 400000);

				if (pm_qos_request_active(&data->tsp_int_qos))
					pm_qos_update_request(&data->tsp_int_qos, 200000);
				else
					pm_qos_add_request(&data->tsp_int_qos, PM_QOS_DEVICE_THROUGHPUT, 200000);
			} else {
				pm_qos_add_request(&data->tsp_cpu_qos, PM_QOS_CPU_FREQ_MIN,1000000); /* CPU 1000MHz */
				pm_qos_add_request(&data->tsp_mif_qos, PM_QOS_BUS_THROUGHPUT, 400000); /* MIF 400MHz */
			...
			dev_dbg(&data->client->dev, "TSP DVFS On %d %d %d\n",
				data->boost_level, data->dvfs_lock_status, mode);
...
static void report_input_data(struct mxt224e_data *data)
{
	int i;
	...
	for (i = 0; i < data->num_fingers; i++) {
		if (data->fingers[i].z == TSP_STATE_INACTIVE)
			continue;
...
#if TOUCH_BOOSTER
	if (!!touch_is_pressed)
		set_dvfs_lock(data, 1, check_press);
	else
		set_dvfs_lock(data, 0, false);
#endif
}
...
static int mxt224_internal_suspend(struct mxt224e_data *data)
{
	int i;
	int ret = 0;
	...
	for (i = 0; i < data->num_fingers; i++) {
		...
		data->touch_is_pressed_arr[i] = TSP_STATE_RELEASE;
		data->fingers[i].z = 0;
	}
	report_input_data(data);
...
static int mxt224_internal_resume(struct mxt224e_data *data)
{
	int ret = 0;
	bool ta_status = 0;
...
{
	struct mxt224e_data *data = container_of(h, struct mxt224e_data,
						early_suspend);
...
static int mxt224_suspend(struct device *dev)
{
	...
	struct mxt224e_data *data = i2c_get_clientdata(client);
	int ret = 0;
...
static ssize_t mxt224_object_setting(struct device *dev,
				       ...
				       const char *buf, size_t count)
{
	struct mxt224e_data *data = dev_get_drvdata(dev);
...
static void boost_level(void *device_data)
{
	struct mxt224e_data *data = (struct mxt224e_data *)device_data;
	struct i2c_client *client = data->client;
	...
	if (fdata->cmd_param[0] < 0 || fdata->cmd_param[0] > 2) {
		...
		data->boost_level = fdata->cmd_param[0];
		dev_dbg(&client->dev, "%s %d\n", __func__, data->boost_level);
...
static bool mxt_check_xy_range(struct mxt224e_data *data, u16 node)
{
...
static int mxt_xy_to_node(struct mxt224e_data *data)
{
	struct i2c_client *client = data->client;
	struct mxt_fac_data *fdata = data->fdata;
...
static void mxt_node_to_xy(struct mxt224e_data *data, u16 *x, u16 *y)
{
	struct i2c_client *client = data->client;
	struct mxt_fac_data *fdata = data->fdata;
...
static int mxt_set_diagnostic_mode(struct mxt224e_data *data, u8 dbg_mode)
{
	struct i2c_client *client = data->client;
	u8 cur_mode;
	int ret;
...
static void mxt_treat_dbg_data(struct mxt224e_data *data,
...
static int mxt_read_all_diagnostic_data(struct mxt224e_data *data, u8 dbg_mode)
{
	struct i2c_client *client = data->client;
	struct mxt_fac_data *fdata = data->fdata;
	u16 size;
...
static int mxt_load_fw_from_ums(struct mxt_fw_info *fw_info,
		const u8 *fw_data)
{
	struct mxt224e_data *data = fw_info->data;
...
{
	struct mxt224e_data *data = input_get_drvdata(dev);
...
static void mxt_deallocate_factory(struct mxt224e_data *data)
{
	struct mxt_fac_data *fdata = data->fdata;
...
static int mxt_allocate_factory(struct mxt224e_data *data)
{
	const struct mxt224e_platform_data *pdata = data->pdata;
...
static int mxt_init_factory(struct mxt224e_data *data)
{
	struct device *dev = &data->client->dev;
	int error;
...
static void mxt_exit_factory(struct mxt224e_data *data)
{
	struct mxt_fac_data *fdata = data->fdata;
...
static void  mxt_sysfs_remove(struct mxt224e_data *data)
{
				  ...
				  const struct i2c_device_id *id)
{
	...
	data->callbacks.inform_charger = mxt224_ta_cb;
	if (data->register_cb)
	...
	data->boost_level = TSP_BOOSTER_LEVEL2;
#endif
	...
	printk(KERN_ERR"[TSP] probe done\n");
	return 0;
...
static void mxt224_shutdown(struct i2c_client *client)
{
	struct mxt224e_data *data = i2c_get_clientdata(client);

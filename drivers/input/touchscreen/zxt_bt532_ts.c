...
static bool get_raw_data(struct bt532_ts_data *touch_dev, u8 *buff,
			 int skip_cnt)
{
	struct i2c_client *client = touch_dev->client;
	struct zxt_ts_platform_data *pdata = touch_dev->pdata;
...
static bool ts_read_coord(struct bt532_ts_data *touch_dev)
{
...
#if (TOUCH_POINT_MODE == 1)
	memset(&touch_dev->touch_info, 0x0, sizeof(struct point_info));
	...
	dev_info(&client->dev, "status reg = 0x%x , event_flag = 0x%04x\n",
		 touch_dev->touch_info.status,
		 touch_dev->touch_info.event_flag);
	...
	for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
		if (zinitix_bit_test(touch_dev->touch_info.event_flag, i)) {
			...
			if (read_data
			    (touch_dev->client,
			     ZXT_POINT_STATUS_REG + 2 + (i * 4),
			     (u8 *) (&touch_dev->touch_info.coord[i]),
			     sizeof(struct coord)) < 0) {
				dev_err(&client->dev,
					"Failed to read point touch_dev\n");
...
static void ts_tmr_work(struct work_struct *work)
{
	...
	disable_irq(touch_dev->irq);
	zinitix_printk
	    ("error. timeout occured. maybe ts device dead. so reset & reinit.\n");
...
static bool ts_check_need_upgrade(struct bt532_ts_data *touch_dev,
				  u16 cur_ver, u16 cur_minor_ver,
				  u16 cur_reg_ver, u16 cur_hw_id)
{
	u16 new_ver;
	u16 new_minor_ver;
	u16 new_reg_ver;
	...
	new_minor_ver =
	    (u16) (m_firmware_data[56] | (m_firmware_data[57] << 8));
	new_reg_ver = (u16) (m_firmware_data[60] | (m_firmware_data[61] << 8));
	new_chip_code =
	    (u16) (m_firmware_data[64] | (m_firmware_data[65] << 8));

	new_hw_id =
	    (u16) (m_firmware_data[0x6b12] | (m_firmware_data[0x6b13] << 8));
 ...
 retry_upgrade:
	...
	for (flash_addr = 0; flash_addr < size;) {
		for (i = 0; i < page_sz / TC_SECTOR_SZ; i++) {
			...
			if (write_data
			    (client, ZXT_WRITE_FLASH,
			     (u8 *) &firmware_data[flash_addr],
			     TC_SECTOR_SZ) < 0) {
				zinitix_printk(KERN_INFO
	...
	if (memcmp((u8 *) &firmware_data[0], (u8 *) &verify_data[0], size) ==
	    0) {
...
static bool ts_hw_calibration(struct bt532_ts_data *touch_dev)
{
	...
	if (write_reg(client, ZXT_TOUCH_MODE, 0x07) != I2C_SUCCESS)
		return false;
	mdelay(10);
	write_cmd(client, ZXT_CLEAR_INT_STATUS_CMD);
	mdelay(10);
	...
	if (write_cmd(client, ZXT_CLEAR_INT_STATUS_CMD) != I2C_SUCCESS)
		return false;
	...
	if (write_reg(client, ZXT_TOUCH_MODE, TOUCH_POINT_MODE) != I2C_SUCCESS)
		return false;
	...
	udelay(100);
	if (write_cmd(client, ZXT_SAVE_CALIBRATION_CMD) != I2C_SUCCESS)
		return false;
 ...
 retry_init:
	for (i = 0; i < INIT_RETRY_CNT; i++) {
		if (read_data(client, ZXT_EEPROM_INFO_REG,
			      (u8 *) &chip_eeprom_info, 2) < 0) {
			dev_err(&client->dev,
				"Failed to read eeprom touch_dev(%d)\n", i);
			mdelay(10);
			continue;
		} else
			break;
	...
	if (read_data(client, ZXT_VENDOR_ID, (u8 *) &cap->vendor_id, 2) < 0) {
	...
	if (read_data(client, ZXT_SENSITIVITY, (u8 *) &cap->threshold, 2) < 0)
		goto fail_init;
	...
	if (write_reg(client,
		      ZXT_INT_ENABLE_FLAG, cap->ic_int_mask) != I2C_SUCCESS)
		goto fail_init;
	...
	if (touch_dev->touch_mode != TOUCH_POINT_MODE) {	/* Test Mode */
		if (write_reg(client,
			...
			zinitix_printk
			    ("Fail to set ZXT_DELAY_RAW_FOR_HOST.\n");
			goto fail_init;
		}
	}
#if ESD_TIMER_INTERVAL
	...
	read_data(client, ZXT_PERIODICAL_INTERRUPT_INTERVAL, (u8 *) &reg_val,
		  2);
...
static bool mini_init_touch(struct bt532_ts_data *touch_dev)
{
	...
	if (touch_dev->touch_mode != TOUCH_POINT_MODE) {
		if (write_reg(client, ZXT_DELAY_RAW_FOR_HOST,
			      RAWDATA_DELAY_FOR_HOST) != I2C_SUCCESS) {
			dev_err(&client->dev,
				"Failed to set ZXT_DELAY_RAW_FOR_HOST\n");
...
static irqreturn_t bt532_touch_work(int irq, void *data)
{
	...
	if (ts_read_coord(touch_dev) == false || touch_dev->touch_info.status == 0xffff || touch_dev->touch_info.status == 0x1) {	/* maybe desirable reset */
	...
	if (zinitix_bit_test(touch_dev->touch_info.status, BIT_ICON_EVENT)) {
		if (read_data(touch_dev->client, ZXT_ICON_STATUS_REG,
			      (u8 *) (&touch_dev->icon_event_reg), 2) < 0) {
			dev_err(&client->dev,
				"Failed to read button touch_dev\n");
			write_cmd(client, ZXT_CLEAR_INT_STATUS_CMD);
	...
	if (reported == true ||
		...
		memset(&touch_dev->reported_touch_info, 0x0,
		       sizeof(struct point_info));
		input_sync(touch_dev->input_dev);
...
//                              touch_dev->touch_info.coord[i].minor_width = PALM_REJECT_WIDTH;
			...
			input_report_abs(touch_dev->input_dev, ABS_MT_PRESSURE,
					 (u32) w);
			input_report_abs(touch_dev->input_dev,
					 ABS_MT_WIDTH_MAJOR,
					 (u32) ((palm == 1) ? w - 40 : w));
			...
			input_report_abs(touch_dev->input_dev,
					 ABS_MT_TOUCH_MINOR,
					 (u32) touch_dev->touch_info.coord[i].
					 minor_width);
//                      input_report_abs(touch_dev->input_dev,
...
//                      input_report_abs(touch_dev->input_dev, ABS_MT_PALM, 1);
			...
			memset(&touch_dev->touch_info.coord[i], 0x0,
			       sizeof(struct coord));
			input_mt_slot(touch_dev->input_dev, i);
			input_mt_report_slot_state(touch_dev->input_dev,
						   MT_TOOL_FINGER, 0);
	...
	memcpy((char *)&touch_dev->reported_touch_info,
	       (char *)&touch_dev->touch_info, sizeof(struct point_info));
	input_sync(touch_dev->input_dev);
...
#ifdef SEC_FACTORY_TEST
static inline void set_cmd_result(struct bt532_ts_data *touch_dev, char *buff,
				  int len)
{
...
{
	char delim = ':';
	memset(touch_dev->factory_info->cmd_result, 0x00,
	       ARRAY_SIZE(touch_dev->factory_info->cmd_result));
	memcpy(touch_dev->factory_info->cmd_result,
	       touch_dev->factory_info->cmd,
	       strlen(touch_dev->factory_info->cmd));
...
static int init_sec_factory(struct bt532_ts_data *touch_dev)
{
	...
	if (unlikely(!factory_ts_dev)) {
		dev_err(&touch_dev->client->dev,
			"Failed to create factory dev\n");
		ret = -ENODEV;
		goto err_create_device;
	}
#ifdef SUPPORTED_TOUCH_KEY
	factory_tk_dev =
	    device_create(sec_class, NULL, 0, touch_dev, "sec_touchkey");
...
/* #endif */
{
	...
	case TOUCH_IOCTL_VARIFY_UPGRADE_DATA:
		...
		version =
		    (u16) (m_firmware_data[52] | (m_firmware_data[53] << 8));
			  ...
			  const struct i2c_device_id *i2c_id)
{
	...
	ret = bt532_power_control(touch_dev, POWER_ON_SEQUENCE);
	if (ret == 0)

...
static void mms_protect_firm_erase(struct mms_ts_info *info)
{
	struct i2c_client *client = info->client;
	dev_notice(&client->dev, "%s\n", __func__);

	i2c_smbus_write_byte_data(info->client, 0xB0, 0x1);
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < sz; i += EVENT_SZ) {
		...
		/* touch key */
		if (info->pdata->touchkey && (tmp[0] & 0x40)) {
...
#else
			dev_notice(&client->dev,
				"P [%2d],([%3d],[%4d]) w=%d, major=%d, minor=%d, angle=%d, palm=%d",
				id, x, y, tmp[4], tmp[6], tmp[7], angle, palm);
#endif /* CONFIG_SAMSUNG_PRODUCT_SHIP */
		}
	}
...
static int get_fw_version(struct mms_ts_info *info)
{
	...
	dev_info(&info->client->dev,
		...
		buf[0], buf[1], buf[2]);
	printk(KERN_ERR "boot : 0x%x, core : 0x%x, config : 0x%x\n",
		buf[0], buf[1], buf[2]);
	return 0;
}
...
static int mms_isc_erase_page(struct mms_ts_info *info, int page)
{
	...
	ret |= mms_isc_read_status(info,
		ISC_PAGE_ERASE_DONE | ISC_PAGE_ERASE_ENTER | page);
				...
				struct device_attribute *attr, char *buf)
{
	...
	r_buf = i2c_smbus_read_byte_data(info->client, 0x20);
				  ...
				  const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ARRAY_SIZE(tsp_cmd); i++)
		list_add_tail(&tsp_cmd[i].list, &info->cmd_list_head);

...
static void change_dvfs_lock(struct work_struct *work)
{
	...
	if (TSP_BOOSTER_LEVEL1 == info->boost_level) {
		dev_dbg(&info->client->dev, "%s & Off all\n", __func__);
		...
		if (pm_qos_request_active(&info->tsp_cpu_qos))
			pm_qos_update_request(&info->tsp_cpu_qos, 800000); /* CPU 800MHz */

		if (pm_qos_request_active(&info->tsp_mif_qos))
			pm_qos_update_request(&info->tsp_mif_qos, 267000); /* MIF 267MHz */

		if (pm_qos_request_active(&info->tsp_int_qos))
			pm_qos_update_request(&info->tsp_int_qos, 160000); /* INT 160MHz */

		dev_dbg(&info->client->dev, "%s called!\n", __func__);
	}
...
static void set_dvfs_off(struct work_struct *work)
{
	...
	dev_dbg(&info->client->dev, "TSP DVFS Off %d\n", info->boost_level);
	}

static void set_dvfs_lock(struct mms_ts_info *info, uint32_t on, bool mode)
{
	...
	if (on == 0) {
		...
		if (!info->dvfs_lock_status || mode) {
			if (info->dvfs_lock_status && mode) {
				...
				if (pm_qos_request_active(&info->tsp_cpu_qos))
					pm_qos_update_request(&info->tsp_cpu_qos, 1000000);
				else
					pm_qos_add_request(&info->tsp_cpu_qos, PM_QOS_CPU_FREQ_MIN, 1000000);

				if (pm_qos_request_active(&info->tsp_mif_qos))
					pm_qos_update_request(&info->tsp_mif_qos, 400000);
				else
					pm_qos_add_request(&info->tsp_mif_qos, PM_QOS_BUS_THROUGHPUT, 400000);

				if (pm_qos_request_active(&info->tsp_int_qos))
					pm_qos_update_request(&info->tsp_int_qos, 200000);
				else
					pm_qos_add_request(&info->tsp_int_qos, PM_QOS_DEVICE_THROUGHPUT, 200000);
			} else {
				pm_qos_add_request(&info->tsp_cpu_qos, PM_QOS_CPU_FREQ_MIN,1000000); /* CPU 1000MHz */
				pm_qos_add_request(&info->tsp_mif_qos, PM_QOS_BUS_THROUGHPUT, 400000); /* MIF 400MHz */
			...
			dev_dbg(&info->client->dev, "TSP DVFS On %d %d %d\n",
				info->boost_level, info->dvfs_lock_status, mode);
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	int i;
		int sz = 0;
		u8 buf[(MAX_FINGERS + MAX_KEYS) * EVENT_SZ] = { 0 };
	...
	if (sz == 0)
		...
		if (sz > (MAX_FINGERS + MAX_KEYS) * EVENT_SZ) {
			dev_err(&client->dev, "abnormal data inputed (%d)\n",
				(MAX_FINGERS + MAX_KEYS) * EVENT_SZ);
		goto out;
	}
	...
	if (buf[0] == 0x0E) { /* NOISE MODE */
		...
		for (i = 0; i < sz; i += EVENT_SZ) {
			u8 *tmp = &buf[i];
			int id, x, y; // , angle, palm, width;

			/* touch key */
			if (info->pdata->touchkey && !(tmp[0] & 0x20)) {
				u8 keycode = tmp[0] & 0xf;
				u8 key_press = (tmp[0] >> 7);
			...
			dev_notice(&client->dev,
						"key R : %d(%d)\n",
						info->keycode[keycode], tmp[0] & 0xf);
#endif
		} else {
			input_report_key(info->input_dev,
...
#else
			dev_notice(&client->dev,
					"P [%2d],([%3d],[%4d]) w=%d, major=%d, minor=%d\n",
...
#if TOUCH_BOOSTER
		if (!!touch_is_pressed)
			set_dvfs_lock(info, 1, booster_status);
		else
...
static int mms100_open_mbinary(struct mms_ts_info *info)
{
	...
	ret += request_firmware(&(fw_mbin[0]),\
			"tsp_melfas/delos/BOOT.fw", &_client->dev);
	ret += request_firmware(&(fw_mbin[1]),\
			"tsp_melfas/delos/CORE.fw", &_client->dev);
	ret += request_firmware(&(fw_mbin[2]),\
			"tsp_melfas/delos/CONF.fw", &_client->dev);
	if (!ret)
		return ISC_SUCCESS;
	else {
...
static int mms_flash_boot(struct mms_ts_info *info)
{
	...
	ret = request_firmware(&tsp_fw,
		"tsp_melfas/delos/mms_boot.fw", &(client->dev));
	if (ret) {
...
static int mms_ts_core_fw_load(struct mms_ts_info *info)
{
	...
	ret = request_firmware(&tsp_fw,
		"tsp_melfas/delos/melfas.fw", &(client->dev));
	if (ret) {
		dev_err(&client->dev, "request firmware error!!\n");
		return 1;
	}
...
static void get_fw_ver_bin(void *device_data)
{
	...
	set_default_result(info);
	snprintf(buff, sizeof(buff), "ME%02x53%02x",
		info->panel, FW_VERSION);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void get_config_ver(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "E500_Me_28");
...
static int mms_ts_fw_check(struct mms_ts_info *info)
{
	...
	info->fw_core_ver = get_fw_version(info, SEC_CORE);
	dev_err(&client->dev, "core version : 0x%02x\n",
		info->fw_core_ver);
	info->fw_ic_ver = get_fw_version(info, SEC_CONFIG);

...
//#include "zinitix_touch_zxt_firmware_01(ZI012202).h"
#include "zxt_bt432_fw_01.h"
#include "zxt_bt432_fw_02.h"
...
static struct i2c_board_info __initdata Zxt_i2c_info[] =
	{
		I2C_BOARD_INFO(ZINITIX_TS_NAME, 0x40>>1),
	};
#endif
...
static void ts_select_type_hw(struct bt432_ts_data *touch_dev) {
	int i;
	u16 newHWID;
		...
		const struct i2c_device_id *i2c_id)
{
	...
	touch_dev->input_dev->name = ZINITIX_TS_NAME;
	...
	ret = request_threaded_irq(touch_dev->irq, NULL, zinitix_touch_work,
		IRQF_TRIGGER_FALLING | IRQF_ONESHOT , ZINITIX_TS_NAME, touch_dev);

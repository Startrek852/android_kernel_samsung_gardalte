...
retry:
	...
	pr_info("%s - self test x = %d, y = %d, z = %d\n",
		__func__, x, y, z);
	if ((x > -30) && (x < 30))
		pr_info("%s - x passed self test, -200<=x<=200\n",
			__func__);
	else
		pr_info("%s - x failed self test, -200<=x<=200\n",
			__func__);
	if ((y > -30) && (y < 30))
		pr_info("%s - y passed self test, -200<=y<=200\n",
			__func__);
	else
		pr_info("%s - y failed self test, -200<=y<=200\n",
			__func__);
	if ((z > -400) && (z < -50))
		pr_info("%s - z passed self test, -3200<=z<=-800\n",
			__func__);
	else
		pr_info("%s - z failed self test, -3200<=z<=-800\n",
			__func__);
...
retry_adc:
	...
	ret = ak09911c_smbus_read_byte_block(data->client, AK09911C_REG_ST1,
...
static int ak09911c_read_fuserom(struct ak09911c_p *data)
{
	...
	if (ret < 0) {
		pr_err("%s - unable to load factory sensitivity "\
			"adjust values\n", __func__);
		goto exit_default_value;
	} else
		...
		const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("%s - ak09911c_check_device fail"\
			"(err=%d)\n", __func__, ret);

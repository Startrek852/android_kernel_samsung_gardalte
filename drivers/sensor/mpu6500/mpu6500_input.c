...
#endif
/*static int mpu6500_setup_irq(struct mpu6500_input_data *data){
	int rc = -EIO;
	int irq;
	if (data->client->irq)
		irq = data->client->irq;
	else {
		rc = gpio_request(gb_mpu_data->pdata->gpio_gyro_irq, "gpio_gyro_int");
		if (rc < 0) {
			pr_err("%s: gpio %d request failed (%d)\n",
				__func__, gb_mpu_data->pdata->gpio_gyro_irq, rc);
			return rc;
		}

		rc = gpio_direction_input(gb_mpu_data->pdata->gpio_gyro_irq);
		if (rc < 0) {
		...
		irq = gpio_to_irq(gb_mpu_data->pdata->gpio_gyro_irq);
	}
	data->client->irq = irq;
...
err_request_irq:
	gpio_free(gb_mpu_data->pdata->gpio_gyro_irq);
done:
	return rc;

...
static int es305_set_passthrough(struct es305_data *es305, bool passthrough)
{
	int i, ret;

	if (passthrough == false) {
		ret = es305_send_cmd(es305, ES305_DIGITAL_PASSTHROUGH, NULL);
		if (ret < 0) {
			dev_err(es305->dev, "Failed to change active mode\n");
			return ret;
		}
	...
	for (i = 0; i < es305->pdata->passthrough_num; i++) {
		ret = es305_send_cmd(es305, ES305_DIGITAL_PASSTHROUGH
				 | es305->pdata->passthrough_cmd[i], NULL);
		if (ret < 0) {
			dev_err(es305->dev, "Failed to chagne bypass mode\n");
			return ret;
		}
	}
...
static int es305_sleep(struct es305_data *es305)
{
	...
	es305_set_passthrough(es305, true);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	es305_set_passthrough(es305, false);

...
#ifdef DEBUG_MODEM_IF
	ch = iod->id;
	snprintf(tag, MIF_MAX_STR_LEN, "IOD: %s->%s", iod->name, iod->mc->name);
#if 1
	if (ch >= SIPC5_CH_ID_RFS_0 && ch <= SIPC5_CH_ID_RFS_9)
		pr_ipc(0, tag, skb->data, (skb->len > 20 ? 20 : skb->len));
#endif
#if 0
	else if (ch >= SIPC5_CH_ID_FMT_0 && ch <= SIPC5_CH_ID_FMT_9)
...
#ifdef DEBUG_MODEM_IF
	ch = iod->id;
	snprintf(tag, MIF_MAX_STR_LEN, "IOD: %s->%s", iod->mc->name, iod->name);
#if 1
	if (ch >= SIPC5_CH_ID_RFS_0 && ch <= SIPC5_CH_ID_RFS_9)
		pr_ipc(0, tag, skb->data, (skb->len > 20 ? 20 : skb->len));
#endif

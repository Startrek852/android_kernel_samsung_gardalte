...
static void msg_rx_work(struct work_struct *ws)
{
	...
	for (i = 0; i < MAX_SIPC5_DEV; i++) {
		struct io_device *iod = shmd->iod[i];
		while (1) {
			struct sk_buff *skb = skb_dequeue(ld->skb_rxq[i]);
			if (!skb)
				break;
...
#ifdef CONFIG_TARGET_LOCALE_USA
			   ...
			   struct io_device *iod, unsigned long arg)
{
	...
	err = copy_from_user(dst, (const void __user *)mf.binary, mf.size);
	if (err) {
		       ...
		       unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_MODEM_GET_SHMEM_INFO:
	{
		...
		if (copy_to_user((void __user *)arg, &mem_info, size))
			return -EFAULT;
		break;
	}
...
struct link_device *shmem_create_link_device(struct platform_device *pdev)
{
	...
	*/
	...
	shmd->start = shm_get_phys_base() + shm_get_ipc_rgn_offset();
	shmd->size = shm_get_ipc_rgn_size();

...
static void sm5502_disable_interrupt(struct sm5502_muic_data *muic_data)
{
	...
	value = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_CTRL);
	value |= CTRL_INT_MASK_MASK;

	ret = i2c_smbus_write_byte_data(i2c, SM5502_MUIC_REG_CTRL, value);
	if (ret < 0)
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	ret = i2c_smbus_read_byte_data(muic_data->i2c, SM5502_MUIC_REG_ADC);
	if (ret < 0) {
					   ...
					   char *buf)
{
	...
	ret = i2c_smbus_read_byte_data(muic_data->i2c, SM5502_MUIC_REG_INTMASK2);
					  ...
					  const char *buf, size_t count)
{
	...
	val = i2c_smbus_read_byte_data(muic_data->i2c, SM5502_MUIC_REG_INTMASK2);
	val = i2c_smbus_write_byte_data(muic_data->i2c, 
...
static int set_ctrl_reg(struct sm5502_muic_data *muic_data, int shift, bool on)
{
	...
	ret = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_CTRL);
	if (ret < 0)
	...
	if (reg_val ^ ret) {
		...
		ret = i2c_smbus_write_byte_data(i2c, SM5502_MUIC_REG_CTRL,
				reg_val);
		if (ret < 0)
			...
			enum sm5502_reg_manual_sw1_value reg_val)
{
	...
	temp = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_MANSW1);
	if (temp < 0)
	...
	if (reg_val ^ temp) {
		...
		ret = i2c_smbus_write_byte_data(i2c, SM5502_MUIC_REG_MANSW1,
				reg_val);
		if (ret < 0)
			...
			enum muic_attached_dev new_dev, int adc, u8 vbvolt)
{
	...
	case ATTACHED_DEV_OTG_MUIC:
		...
		break;
	case ATTACHED_DEV_AUDIODOCK_MUIC:
		ret = attach_audiodock(muic_data, new_dev);
		break;
	case ATTACHED_DEV_TA_MUIC:
...
/*  don't call filter_vps_cb, since filter_vps_cb is empty

	if(switch_data->filter_vps_cb)
		switch_data->filter_vps_cb(muic_data);
*/

	val1 = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_DEV_T1);
	if (val1 < 0)
		pr_err("%s:%s err %d\n", MUIC_DEV_NAME, __func__, val1);

	val2 = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_DEV_T2);
	if (val2 < 0)
		pr_err("%s:%s err %d\n", MUIC_DEV_NAME, __func__, val2);

	val3 = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_DEV_T3);
	if (val3 < 0)
		pr_err("%s:%s err %d\n", MUIC_DEV_NAME, __func__, val3);
	else
		vbvolt = (val3 & DEV_TYPE3_VBVOLT);

	adc = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_ADC);
	...
	if(intr != MUIC_INTR_ATTACH) switch (adc) {
...
static irqreturn_t sm5502_muic_irq_thread(int irq, void *data)
{
	...
	sm5502_disable_interrupt(muic_data);

	/* read and clear interrupt status bits */
	intr1 = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_INT1);
	intr2 = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_INT2);
...
skip_detect_dev:
	sm5502_enable_interrupt(muic_data);
...
static int sm5502_init_rev_info(struct sm5502_muic_data *muic_data)
{
	...
	dev_id = i2c_smbus_read_byte_data(muic_data->i2c, SM5502_MUIC_REG_DEVID);
	if (dev_id < 0) {
...
static int sm5502_muic_reg_init(struct sm5502_muic_data *muic_data)
{
	 ...
	 */
	ret = i2c_smbus_write_byte_data(i2c, SM5502_MUIC_REG_INTMASK1,
			REG_INTMASK1_VALUE);
	if (ret < 0)
		pr_err("%s: err mask interrupt1(%d)\n", __func__, ret);

	ret = i2c_smbus_write_byte_data(i2c, SM5502_MUIC_REG_INTMASK2,
			REG_INTMASK2_VALUE);
	if (ret < 0)
		pr_err("%s: err mask interrupt1(%d)\n", __func__, ret);

	ret = i2c_smbus_read_byte_data(i2c, SM5502_MUIC_REG_MANSW1);
	if (ret < 0)
	...
	ret = i2c_smbus_write_byte_data(i2c, SM5502_MUIC_REG_CTRL, ctrl);
	if (ret < 0)
